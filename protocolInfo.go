...
// -- Protocol parsing
func parseProtocol(p string) (noise.HandshakePattern, noise.DHFunc, noise.CipherFunc, noise.HashFunc, error) {
	...
	results := regEx.FindStringSubmatch(p)
	if len(results) == 5 {
		if hs, ok = getConfig(protoInfo{HandshakePatterns: handshakePatterns}, "HandshakePatterns", results[1]).(noise.HandshakePattern); ok == false {
			err = errors.New("Invalid handshake pattern")
			goto exit
		}
		if dh, ok = getConfig(protoInfo{DHFuncs: dhFuncs}, "DHFuncs", results[2]).(noise.DHFunc); ok == false {
			err = errors.New("Invalid DH function")
			goto exit
		}
		if cipher, ok = getConfig(protoInfo{CipherFuncs: cipherFuncs}, "CipherFuncs", results[3]).(noise.CipherFunc); ok == false {
			err = errors.New("Invalid cipher function")
			goto exit
		}
		if hash, ok = getConfig(protoInfo{HashFuncs: hashFuncs}, "HashFuncs", results[4]).(noise.HashFunc); ok == false {
...
func getConfig(p protoInfo, field string, search string) interface{} {
